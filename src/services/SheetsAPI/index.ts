...
class SheetsAPI {
    ...
    async get_sheet() {
        ...
        await sheet.setHeaderRow(this.accounts.map((account) => String(account.id)));
    ...
    logger = {
        ...
        log: async (message: string, profile_id: number) => {
            const sheet = await this.get_sheet();

            const column = sheet.headerValues.indexOf(String(profile_id));
        ...
        info: async (message: string, profile_id: number)=> this.logger.log(`[INFO] [${this.logger.datetime()}]: ${message}`, profile_id),
        error: async (message: string, profile_id: number)=> this.logger.log(`[ERROR] [${this.logger.datetime()}]: ${message}`, profile_id),
        warning: async (message: string, profile_id: number)=> this.logger.log(`[WARNING] [${this.logger.datetime()}]: ${message}`, profile_id),
    }
}
